...
subroutine init_beta(lo, hi, beta, betalo, betahi, dx, prob_lo, prob_hi,epsilon, k_freq, Nprob) bind(C, name="init_beta")
  !  Initialize the scalar field beta
  use amrex_fort_module, only : amrex_real
  
  implicit none
  
  integer, intent(in) :: lo(2), hi(2), betalo(2), betahi(2), Nprob
  real(amrex_real), intent(inout) :: beta(betalo(1):betahi(1),betalo(2):betahi(2))
  real(amrex_real), intent(in   ) :: dx(2)
  real(amrex_real), intent(in   ) :: prob_lo(2)
  real(amrex_real), intent(in   ) :: prob_hi(2)
  ...
  if (epsilon .gt. 0.0) then
     do j = betalo(2), betahi(2)
        y = prob_lo(2) + dble(j) * dx(2)
        do i = betalo(1), betahi(1)
           x = prob_lo(1) + dble(i) * dx(1)
           do j_quad = 0,4
              y_quad = y + dx(2)*gauss_nodeFrac(j_quad)
              do i_quad = 0,4
                 x_quad = x + dx(1)*gauss_nodeFrac(i_quad)
                 if ((Nprob .EQ. 1) .OR. (Nprob .EQ. 4)) then
                    beta(i,j) = beta(i,j)+ gauss_weights(j_quad)*gauss_weights(i_quad)* &
                         (epsilon*sin(om*(x_quad))*sin(om*(y_quad)))
                 elseif (Nprob .EQ. 3) then
                    beta(i,j) = beta(i,j)+ gauss_weights(j_quad)*gauss_weights(i_quad)* &
                         (epsilon*sin(om*(x_quad+y_quad)))
                 endif
              end do
           end do
        end do
